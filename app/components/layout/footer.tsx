      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <div className="relative grid gap-10 px-8 py-12 md:px-12 lg:grid-cols-[1.4fr_0.6fr] lg:py-16">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-6 text-4xl font-bold leading-[1.1] text-white sm:text-5xl lg:text-6xl">
                Elevá tu entrenamiento a una <span className="italic font-serif text-[#D4AF37]">experiencia</span> superior.
              </h2>
            </div>
        ...
        <section className="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-6">
            <Link href="/" className="text-3xl font-bold tracking-[0.3em] text-[#2D241E]">
              SHINY
            </Link>
            <p className="text-sm leading-8 text-[#5C4D42]">
              Boutique deportiva pensada para mujeres que transforman su energía en movimiento. Calidad premium en cada costura.
            </p>
            <div className="flex gap-4">
              <a href="#" className="flex h-10 w-10 items-center justify-center rounded-full border border-[#E5DED4] transition-colors hover:bg-[#2D241E] hover:text-white">
          ...
          <div>
            <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#8B5E3C]">Navegación</h4>
            <ul className="mt-8 space-y-4">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="group flex items-center text-sm font-medium text-[#4A3728] transition-colors hover:text-[#8B5E3C]">
          ...
          {/* Col 3: Contacto */}
          <div>
            <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#8B5E3C]">Contacto Directo</h4>
            <div className="mt-8 space-y-6">
              {contactItems.map((item) => (
                <a key={item.label} href={item.href} className="group block">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[#B79E89]">{item.label}</p>
                  <p className="mt-1 text-sm text-[#2D241E] underline-offset-4 group-hover:underline">{item.value}</p>
                </a>
              ))}
              <div className="flex items-start gap-2">
                <MapPin className="h-4 w-4 shrink-0 text-[#8B5E3C]" />
                <p className="text-sm text-[#2D241E]">Showroom en Córdoba, Argentina. <br/><span className="text-[10px] text-[#B79E89]">Atención con cita previa.</span></p>
              </div>
            </div>
          </div>

          {/* Col 4: Newsletter Premium */}
          <div className="rounded-[2.5rem] bg-[#F5F0EB] p-8">
            <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#8B5E3C]">Newsletter</h4>
            <p className="mt-4 text-sm font-bold text-[#2D241E]">Sumate al Club Shiny.</p>
            <form className="mt-6 space-y-3">
              <input
                type="email"
                placeholder="Email"
                className="w-full border-b border-[#DCCBC] bg-transparent py-2 text-sm outline-none placeholder:text-[#B79E89] focus:border-[#2D241E]"
              />
              <button className="flex w-full items-center justify-between text-[10px] font-bold uppercase tracking-widest text-[#2D241E] transition-transform hover:translate-x-1">
        ...
        <section className="mt-20 flex flex-col items-center justify-between gap-6 border-t border-[#E5DED4] pt-8 md:flex-row">
          ...
          <div className="flex gap-8 text-[10px] font-bold uppercase tracking-widest text-[#2D241E]">
            <Link href="/privacidad" className="hover:text-[#8B5E3C]">Privacidad</Link>
            <Link href="/terminos" className="hover:text-[#8B5E3C]">Términos</Link>
            <Link href="/envios" className="hover:text-[#8B5E3C]">Envíos</Link>
          </div>
        </section>
      </div>
    </footer>
